...
#### 3.1 SQL执行服务（3天）
- [ ] 复用现有的SQL执行逻辑（interface_executor.py）
- [ ] 实现SQL执行结果处理
  - 结果格式化（JSON）
  - 数据类型转换
  - 空值处理
...
- [ ] 消息展示组件
  - 用户消息展示
  - AI回复展示（包含SQL、图表、分析结果）
  - 消息时间戳
  - 消息操作按钮（复制SQL、查看明细、导出等）
...
#### 3.5 数据导出功能（2天）
- [ ] 后端导出API
  - POST /api/v1/chat/messages/{id}/export
  - 支持Excel导出
  - 支持CSV导出
  - 支持PNG导出（图表）
...
- [ ] 仪表板组件API
  - POST /api/v1/dashboards/{id}/widgets - 添加组件
  - PUT /api/v1/dashboards/{id}/widgets/{widget_id} - 更新组件
  - DELETE /api/v1/dashboards/{id}/widgets/{widget_id} - 删除组件
