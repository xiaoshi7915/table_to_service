...
class DeepSeekClient(BaseLLMClient):
    ...
    def __init__(self, api_key: str, api_base_url: Optional[str] = None, model_name: str = "deepseek-chat", **kwargs):
        """
        初始化DeepSeek客户端
        
        Args:
            api_key: DeepSeek API密钥
            api_base_url: API基础URL（默认：https://api.deepseek.com）
            model_name: 模型名称（默认：deepseek-chat）
        """
        super().__init__(api_key, api_base_url, model_name, **kwargs)
        
        # DeepSeek的默认API地址
        self.api_base_url = api_base_url or "https://api.deepseek.com"
        ...
        # 初始化token编码器（使用cl100k_base编码，与GPT-3.5/GPT-4相同）
        try:
